...
class LLMPredictor(Predictor):
  def __init__(
    ...
    permanent_attention_cache: Union[bool, str] = False,
    ...
    self.attention_cache_path = get_cache_path(
      permanent_attention_cache, "attention", prompt, model
    )
  ...
  def predict(self, prefix: str) -> Dict[str, float]:
    ...
    cur_next_token = {
      token: float(prob) for token, prob in zip(self.tokens, next_token_probs)
    }
